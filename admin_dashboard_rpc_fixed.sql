-- Função RPC para o painel admin da plataforma (CORRIGIDA)
...
BEGIN
    ...
    -- 5. Buscar Histórico de Taxas da Plataforma (CORRIGIDO)
    SELECT COALESCE(
        json_agg(
            json_build_object(
                'id', ot.id,
    ...
    -- 6. Buscar Eventos sem Taxa de Serviço (CORRIGIDO)
    SELECT COALESCE(
        json_agg(
            json_build_object(
                'id', e.id,
    ...
    AND COALESCE(reg_count.count, 0) > 0;  -- Só eventos com inscrições confirmadas
...
-- Função para gerar taxa manualmente para um evento (mantida igual)
...
COMMENT ON FUNCTION get_admin_dashboard_data() IS 'Retorna dados completos para o dashboard admin: métricas, histórico de taxas e eventos sem taxa (CORRIGIDO)';
